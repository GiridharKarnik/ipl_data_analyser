...
pub fn total_score_of_innings(match_stat_innings: MatchStatInnings) -> u16 {
    ...
    match_stat_innings.overs
        .unwrap()
        .into_iter()
        .for_each(|over: MatchInningsOver| {
            over.deliveries.into_iter().for_each(|delivery: MatchOverDelivery| {
...
pub fn avg_score(
    ...
    filtered_match_stats.into_iter().for_each(|match_stat: MatchStat| {
        let innings_of_intereset = match_stat.innings
            .into_iter()
...
#[wasm_bindgen]
pub fn do_absolutely_nothing() {
    console_error_panic_hook::set_once();
    console::log_1(&JsValue::from_str("This rust function does nothing"));
}
